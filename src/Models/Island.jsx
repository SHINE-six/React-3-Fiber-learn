...
export default function IslandModel({ handleAllowInteract, isRotating, setIsRotating, segment, allowInteract, ...props}) {
  const islandRef = useRef();
  const lastX = useRef();
  const [rotateEdy,setRotateEdy] = useState(false);
  const [segmentChosen, setSegmentChosen] = useState(0);
  const segmentLocation = [0, Math.PI / 2, Math.PI, (Math.PI * 3) / 2, Math.PI * 2];

  const { nodes, materials } = useGLTF("src/Assets/3d/foxs_islands.glb");
  ...
  //* Handle pointer movement
  {
    ...
    const handlePointerMove = (e) => {
      ...
      if (isRotating) {
        const clientX = e.touches ? e.touches[0].clientX : e.clientX;
        const delta = (clientX - lastX.current) / 100;
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key === "ArrowLeft") {
        ...
        islandRef.current.rotation.y += 0.1;
      } 
    ...
    useEffect((e) => {
      ...
      if (allowInteract) {
        ...
        if (!isRotating) {
          const normalizedRotation = Math.abs(islandRef.current.rotation.y % (Math.PI * 2)).toFixed(2);
          const segmentChosen = nearestSegment(normalizedRotation);
          setSegmentChosen(segmentChosen);
          console.log(segmentChosen);
          segment(segmentChosen);
        }
  ...
  function nearestSegment(normalizedRotation) {
    let nearest = 0;
    for (let i = 0; i < segmentLocation.length; i++) {
      if (Math.abs(normalizedRotation - segmentLocation[i]) < Math.abs(normalizedRotation - segmentLocation[nearest])) {
        nearest = i;
      }
    }
    if (nearest === 4) {
  ...
  function animateBackToSegment(segmentChosen) {
    const normalizedRotation = Math.abs(islandRef.current.rotation.y % (Math.PI * 2));
    const delta = (segment[segmentChosen] - normalizedRotation) / 20;
    islandRef.current.rotation.y += delta;
  }

  useFrame(() => {
      if (!isRotating && rotateEdy) {
        // animateBackToSegment(segmentChosen);
      }
...
useGLTF.preload("src/Assets/3d/foxs_islands.glb");
